...
namespace BookingForm.Infra.DAOs
{
	public class UserDAO
	{
		...
		{
			string sql = @" SELECT Count(*) as count FROM Customers WHERE Account=@Account";

			var parameters = new SqlParameterBuilder()
				.AddNVarchar("Account", 300, account)
				.Build();
		...
		{
			string sql = @" SELECT Count(*) as count FROM Customers WHERE Account=@Account AND Id!=@Id";

			var parameters = new SqlParameterBuilder()
				.AddNVarchar("Account", 300, model.Account)
				.AddInt("Id", model.Id)
				.Build();
